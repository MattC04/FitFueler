...
const utils_3 = require("./clients/CognitoIdentityProvider/utils");
...
async function handleCustomChallenge({ challengeResponse, clientMetadata, session, username, config, tokenOrchestrator, }) {
    ...
    const response = await (0, CognitoIdentityProvider_1.respondToAuthChallenge)({
        region: (0, utils_3.getRegion)(userPoolId),
...
async function handleMFASetupChallenge({ challengeResponse, username, clientMetadata, session, deviceName, config, }) {
    ...
    return (0, CognitoIdentityProvider_1.respondToAuthChallenge)({ region: (0, utils_3.getRegion)(userPoolId) }, jsonReq);
}
...
async function handleDeviceSRPAuth({ username, config, clientMetadata, session, tokenOrchestrator, }) {
    ...
    const { ChallengeParameters: respondedChallengeParameters, Session } = await (0, CognitoIdentityProvider_1.respondToAuthChallenge)({ region: (0, utils_3.getRegion)(userPoolId) }, jsonReqResponseChallenge);
...
async function handleDevicePasswordVerifier(username, challengeParameters, clientMetadata, session, authenticationHelper, { userPoolId, userPoolClientId }, tokenOrchestrator) {
    ...
    return (0, CognitoIdentityProvider_1.respondToAuthChallenge)({ region: (0, utils_3.getRegion)(userPoolId) }, jsonReqResponseChallenge);
}
async function handlePasswordVerifierChallenge(password, challengeParameters, clientMetadata, session, authenticationHelper, config, tokenOrchestrator) {
    ...
    const response = await (0, CognitoIdentityProvider_1.respondToAuthChallenge)({ region: (0, utils_3.getRegion)(userPoolId) }, jsonReqResponseChallenge);
    if (response.ChallengeName === 'DEVICE_SRP_AUTH')
        return handleDeviceSRPAuth({
            username,
            config,
...
async function getSignInResult(params) {
    ...
    switch (challengeName) {
        ...
        case 'MFA_SETUP': {
            ...
            const { Session, SecretCode: secretCode } = await (0, CognitoIdentityProvider_1.associateSoftwareToken)({ region: (0, utils_3.getRegion)(authConfig.userPoolId) }, {
...
async function getNewDeviceMetatada(userPoolId, newDeviceMetadata, accessToken) {
    ...
    try {
        await (0, CognitoIdentityProvider_1.confirmDevice)({ region: (0, utils_3.getRegion)(userPoolId) }, {
